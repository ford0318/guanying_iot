...
import os
#寫檔案
def write_station_bkfile(station,data,timestamp):
    # station name, data
    path = "/home/moxa/guanying/csv/" + datetime.datetime.now().strftime("%Y%m%d")
    if not os.path.exists(path):
        os.mkdir(path)
    data.append(timestamp)
    filename = path  + "/" +  datetime.datetime.now().strftime("%Y%m%d") +"_" + station + ".csv"
...
def write_mqtt_file(data):
    # station name, data
    path ="/home/moxa/guanying/mqttcsv/" + datetime.datetime.now().strftime("%Y%m%d")
    if not os.path.exists(path):
        os.mkdir(path)
    filename = path + "/" + datetime.datetime.now().strftime("%Y%m%d") +"_" +"mqtt" + ".csv"
    with open(filename,"a",newline="") as csvfile:
...
if __name__ == "__main__":
    client = ModbusClient('192.168.3.122',port=502)
    time.sleep(3)
